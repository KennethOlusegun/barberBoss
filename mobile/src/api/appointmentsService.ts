...
export const appointmentsService = {
  async updateFinancials(
    id: string,
    data: { finalPrice: number; barberCommission: number },
  ): Promise<Appointment> {
    const res: AxiosResponse<Appointment> = await apiClient.patch(
      `${resource}/${id}/financials`,
      data,
    );
    return res.data;
  },
  ...
  async getBarbers(): Promise<Barber[]> {
    const res: AxiosResponse<Barber[]> = await apiClient.get("/users", {
      params: { role: "BARBER" },
    });
    return res.data;
  },
  ...
  async getAll(params?: Record<string, any>): Promise<AppointmentsResponse> {
    const res: AxiosResponse<AppointmentsResponse> = await apiClient.get(
      resource,
      { params },
    );
    return res.data;
  ...
  async getById(id: string): Promise<Appointment> {
    const res: AxiosResponse<Appointment> = await apiClient.get(
      `${resource}/${id}`,
    );
    return res.data;
  },
  ...
  async create(payload: CreateAppointmentDTO): Promise<Appointment> {
    const res: AxiosResponse<Appointment> = await apiClient.post(
      resource,
      payload,
    );
    return res.data;
  ...
  async checkout(
    id: string,
    data: { paymentMethod: string; finalPrice: number; notes?: string },
  ): Promise<Appointment> {
    const res: AxiosResponse<Appointment> = await apiClient.post(
