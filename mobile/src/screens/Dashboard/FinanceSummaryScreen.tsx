...
export default function FinanceSummaryScreen() {
  ...
  const fetchFinanceSummary = async () => {
    try {
      ...
      const response = await apiClient.get("/finance-report/summary", {
        params,
      });
  ...
  const isEmpty =
    !financeSummary ||
    !financeSummary.appointments ||
    financeSummary.appointments.length === 0;
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          {user?.role === "ADMIN" && (
            <TouchableOpacity
              ...
              style={{ marginRight: 8 }}
              accessibilityLabel="Ir para Acerto de Comissões"
            >
              <MaterialIcons
          ...
          <View style={styles.summaryGrid}>
            ...
            <View style={styles.summaryCol}>
              ...
              <Text style={[styles.summaryColValue, { color: "#4ade80" }]}>
                R${" "}
                {financeSummary ? financeSummary.completed.toFixed(2) : "0,00"}
              </Text>
            </View>
            {/* Divisor vertical */}
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const commission =
                ...
                (item.service?.barberCommission ?? 0.5);
              ...
              const statusIcon = isConfirmed
                ...
                : isPending
                  ? "schedule"
                  : "cancel";
              const commissionColor = isConfirmed
                ? "#4ade80"
                : isPending
                  ? "#fbbf24"
                  : COLORS.grey_steel;
