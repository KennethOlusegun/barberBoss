...
import { View, Text, TouchableOpacity, StyleSheet, FlatList, ActivityIndicator, Alert, StatusBar } from 'react-native';
...
import { appointmentsService, Barber, Appointment } from '../../api/appointmentsService';
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                await Promise.all(barbersList.map(async (barber) => {
                    try {
    ...
    if (user?.role !== 'ADMIN') {
        return (
            <SafeAreaView style={[styles.safeArea, { justifyContent: 'center', alignItems: 'center' }]}>
                <Text style={{ color: COLORS.danger, fontSize: 18, fontWeight: 'bold' }}>Acesso Restrito ao Admin</Text>
            </SafeAreaView>
        );
    }
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.contentContainer}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <Text style={styles.emptyListText}>Nenhum barbeiro encontrado.</Text>
                    }
                    renderItem={({ item }) => {
                        const saldo = (pending[item.id] || []).reduce((acc, ap) => acc + (ap.commission ?? 0), 0);
                        return (
                            <View style={styles.barberCard}>
                                <TouchableOpacity onPress={() => setExpanded(expanded === item.id ? null : item.id)}>
                                    <View style={styles.barberRow}>
                                        <Text style={styles.barberName}>{item.name}</Text>
                                        <Text style={[styles.saldo, { color: saldo > 0 ? COLORS.success : '#9CA3AF' }]}>
                                            R$ {saldo.toFixed(2)}
                                        </Text>
                                    </View>
                                </TouchableOpacity>
                                        ...
                                        <FlatList
                                            ...
                                            renderItem={({ item: ap }) => (
                                                <View style={styles.appointmentRow}>
                                                    <Text style={styles.service} numberOfLines={1}>{ap.serviceName}</Text>
                                                    <Text style={styles.date}>{ap.startsAt ? new Date(ap.startsAt).toLocaleDateString() : '-'}</Text>
                                                    <Text style={styles.commission}>R$ {ap.commission?.toFixed(2)}</Text>
                                                </View>
                                            )}
                                            ListEmptyComponent={<Text style={styles.empty}>Nenhum corte pendente.</Text>}
                                        />
                                        {saldo > 0 && (
                                            <TouchableOpacity style={styles.payoutBtn} onPress={() => handlePayout(item.id)}>
                                                <Text style={styles.payoutText}>Registrar Pagamento</Text>
                                            </TouchableOpacity>
                                        )}
                                    </View>
                                )}
