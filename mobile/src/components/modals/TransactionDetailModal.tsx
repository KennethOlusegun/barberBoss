...
const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  React.useEffect(() => {
    if (transaction) {
      ...
      // @ts-ignore
      setBarberCommission(
        (transaction.commission ?? transaction.service?.price)
          ? transaction.service.price * 0.5
          : 0,
      );
    }
  }, [transaction]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await appointmentsService.updateFinancials(
        transaction.id,
        {
          finalPrice,
          barberCommission,
      ...
      <View style={styles.overlay}>
        <View style={styles.card}>
          ...
          <Text style={styles.value}>
            {transaction.service?.name || "Serviço removido"}
          </Text>

          <Text style={styles.label}>Barbeiro</Text>
          <Text style={styles.value}>
            {transaction.barber?.name || "Não atribuído"}
          </Text>

          <Text style={styles.label}>Data</Text>
          <Text style={styles.value}>
            {transaction.startsAt
              ? new Date(transaction.startsAt).toLocaleString()
              : "-"}
          </Text>
