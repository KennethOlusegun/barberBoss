 ...
 */
export const Throttle = (limit: number, ttl: number) => NestThrottle([{ limit, ttl }]);

/**
 * Rate limiting estrito para operações sensíveis (ex: login, cadastro)
 * 5 requisições por minuto
 */
export const ThrottleStrict = () => NestThrottle([{ name: 'strict', ttl: 60000, limit: 5 }]);

/**
 * Rate limiting moderado para operações comuns
 * 30 requisições por minuto
 */
export const ThrottleModerate = () => NestThrottle([{ name: 'moderate', ttl: 60000, limit: 30 }]);

/**
 * Rate limiting relaxado para leitura
 * 100 requisições por minuto
 */
export const ThrottleRelaxed = () => NestThrottle([{ name: 'relaxed', ttl: 60000, limit: 100 }]);
